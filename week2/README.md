...
이번 2주차 과제에서는 2D 이미지를 Depth Map과 Point Cloud 형태로 변환하고, 해당 기능을 Unit Test로 검증하였다.  
`src/`와 `tests/`를 분리하여 기능 코드와 테스트 코드를 따로 관리하면서 실무적인 코드 구조를 경험할 수 있었다.  
특히 `pytest`를 통해 변환 결과의 크기, 데이터 형태, 예외 처리 여부를 자동으로 확인하며 테스트의 중요성을 이해하였다.  
다만 이번 Depth Map은 실제 거리 정보가 아닌 밝기 기반의 가상 깊이 표현이므로, 향후에는 MiDaS나 Stereo Vision 기반 방식으로 개선할 수 있다.